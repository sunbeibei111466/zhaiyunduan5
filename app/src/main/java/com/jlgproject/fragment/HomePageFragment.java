...
import com.jlgproject.activity.Buness_Pic_Text;

import com.jlgproject.activity.Buness_School2;

import com.jlgproject.activity.H_BusinessSchool;
import com.jlgproject.activity.H_DebtEncyclopedias;
...
import com.jlgproject.activity.Open_Others_Debts;
...
public class HomePageFragment extends BaseFragment implements View.OnClickListener, OnBannerListener, HttpMessageInfo.IMessage, SwipeRefreshLayout.OnRefreshListener {
    ...
    public View initView(View view) {
        refreshLayout = (VpSwipeRefreshLayout) view.findViewById(R.id.refresh);
        ...
        mListView_home.addHeaderView(header,null,false);
    ...
    public void onClick(View v) {
        if (v == ll_home_zsba) {//<!--债事备案-->
            ...
            if (UserInfoState.isLogin()) {
                if(UserInfoState.getUserType()==1 && User.openDebtRew(getActivity(),UserInfoState.getHangType())){
                    ActivityCollector.startA(getActivity(), Open_Others_Debts.class);
                }else{
...
//                ActivityCollector.startA(getActivity(), H_BusinessSchool.class);
                ActivityCollector.startA(getActivity(), Buness_School2.class);
            } else {
                ActivityCollector.startA(getActivity(), Login.class);
            }
...
//            ToastUtil.showShort(getActivity(), "该功能正在开发中");
            if (UserInfoState.isLogin()) {
                startActivity(new Intent(getActivity(),Buness_Pic_Text.class).putExtra("show",8));
            } else {
                ActivityCollector.startA(getActivity(), Login.class);
            }
