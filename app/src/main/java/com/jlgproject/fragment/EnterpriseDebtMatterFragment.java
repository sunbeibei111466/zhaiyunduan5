...
public class EnterpriseDebtMatterFragment extends BaseFragment implements View.OnClickListener {
    ...
    private Add ssq ;//地址
    private String qiyeCode,qiyeFr,qiyeFrCode,jine;
    ...
    private void next() {
        ...
        if (!TextUtils.isEmpty(mEt_organization_code.getText().toString())) {//组织机构代码
            qiyeCode=mEt_organization_code.getText().toString().trim();
        }

        if (!TextUtils.isEmpty(mEt_legal_person_name.getText().toString())) {//法人姓名
            qiyeFr=mEt_legal_person_name.getText().toString().trim();
        }

        if (!TextUtils.isEmpty(mEt_legal_person_icard.getText().toString())) {//法人身份证号
            if(mEt_legal_person_icard.getText().length()==18){
                qiyeFrCode=mEt_legal_person_icard.getText().toString().trim();
            }else{
                ToastUtil.showShort(getActivity(),"请正确填写身份证号");
                return;
            }
        }
