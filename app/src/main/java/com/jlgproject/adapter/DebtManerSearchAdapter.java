...
public class DebtManerSearchAdapter extends BaseAdapter {
    private Context context;

    private List<Debts_Manger.DataBean.ItemsBean> items;
    private Long id;

    public DebtManerSearchAdapter(Context context, List<Debts_Manger.DataBean.ItemsBean> items) {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            vh.tv_recommend_preson= (TextView) convertView.findViewById(R.id.tv_recommend_preson);//推荐人
            vh.tv_hang_type= (TextView) convertView.findViewById(R.id.tv_hang_type);//行长类型
        ...
        vh.tv_recommend_preson.setText(items.get(position).getRecommendName());//推荐人
        vh.tv_hang_type.setText(items.get(position).getHangzhang());//行长类型
        if (payStatus == 0) {//点击付费
    ...
    public class ViewHolder {
        ...
        private TextView entrytime, cretidors, debtor, mainmoney,tv_number,tv_pay_redor,tv_check_the_details,tv_recommend
                ,tv_recommend_preson,tv_hang_type;
    }
}
