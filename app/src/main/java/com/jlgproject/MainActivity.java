...
public class MainActivity extends BaseActivity implements HttpMessageInfo.IMessage {
    ...
    protected void onNewIntent(Intent intent) {
        ...
        L.e("--onNewIntent----index-----" + index+"------newCurrMenu---------**"+newCurrMenu);
    ...
    public void getServiceData(Object o) {
        if (o instanceof VersionBean) {
            ...
            if (versionBean != null) {
                ...
                String service = versionBean.getData().getVersionNum();//服务器版本
                String tishiyu;
                ...
                int serviceVersionCode = versionBean.getData().getVersionCode();//服务器VersionCode
                int lockVersionCode = AppInfoUtil.getVersionCode();//App
                String xiaoVreson;
                if (!TextUtils.isEmpty(versionBean.getData().getContent().toString())) {
                    xiaoVreson = versionBean.getData().getContent().toString();
                } else {
                    xiaoVreson = "应用有更新了";
                }
                if (!service.equals(lockVersion)) { //有更新
                    ...
                    if (lockVersionCode != serviceVersionCode) {
                        DialogUtils.showDialogVersion(this, true, xiaoVreson, new DialogUtils.OnButtonEventListener1() {
                            @Override
                            public void onConfirm() {
                                if (versionBean.getData().getIsForce() == 1) {//强制更新
                                    state = 1;
